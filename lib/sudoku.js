    ...
    */
    solve() {
        ...
        while (!this.solved) {
            ...
            if (
                this.sectors.reduce((changed, sector) => sector.excludeNakedSubsets() || changed,
                    this.columns.reduce((changed, column) => column.excludeNakedSubsets() || changed,
                        this.rows.reduce((changed, row) => row.excludeNakedSubsets() || changed, false)
                    )
                )
            ) {
                continue;
